...
router.get('/:username', authUser, requireLogin, async function(
  ...
  try {
    ...
    // BUG 3
    // had to check the token provided and compare the stored username with the username submitted in the params
...
// BUG FIX 2
router.patch('/admin/edit-user/:username', authUser, requireLogin, requireAdmin, async function(req,res,next) {
  try {
    if (!req.curr_admin && req.curr_username !== req.params.username) {
      throw new ExpressError('Only  that user or admin can edit a user.', 401);
    }
