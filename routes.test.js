...
describe("GET /users/[username]", function() {
  ...
  // I ADDED THIS TEST
  test('Should return a 404 if an invalid name is provided', async function () {
    const response = await request(app)
      .get("/users/u7")
      .send({ _token: tokens.u1 });
    expect(response.statusCode).toBe(404);
...
// BUG FIX 2, CREATED A SEPARATE PATH FOR USERS TO EDIT THEMSELVES
...
// BUG FIX 2, CREATED A NEW ROUTE FOR ADMINS TO EDIT USERS
describe("PATCH /users/admin/edit-user/[username]", function() {
  test("should deny access if no token provided", async function() {
    const response = await request(app).patch("/users/admin/edit-user/u1");
    expect(response.statusCode).toBe(401);
  });

  test("should deny access if not admin/right user", async function() {
    const response = await request(app)
      .patch("/users/admin/edit-user/u1")
      .send({ _token: tokens.u2 }); // wrong user!
    expect(response.statusCode).toBe(401);
  });
  ...
  test("should return 404 if cannot find", async function() {
    const response = await request(app)
      .patch("/users/admin/edit-user/not-a-user")
